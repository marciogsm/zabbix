...
while IFS=";" read -r concat cust hub host ok addr os mrdmon c proxy e status g h i j k l obs; do
        echo "$cust;$addr;$proxy;$host;$os;$status;$obs"
...
# else
    ...
    # Zabbix agent check
    ZBX_OUT=$(zabbix_get -t 5 -s "$addr" -k agent.hostname 2>&1 | awk 'NR==1 {output=$0} NR>1 {output=output"|"$0} END {print output}')
    ZBX_ERROR_CODE="$?"
